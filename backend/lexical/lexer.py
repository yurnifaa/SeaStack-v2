...
class Lexer(
    ...
    def _add_or_error(self, token_type, token_value, line, col, delim_set_name): #Adds a token if the delimiter is valid, otherwise adds a delimiter error
        if self._is_valid_delimiter(delim_set_name):
            ...
            error_msg = f"Invalid Indentifier"

            # NOTE: Custom error messages for common errors should be handled here or by the handler.
